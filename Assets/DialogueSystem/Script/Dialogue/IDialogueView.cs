...
namespace Runtime.Dialogue
{
    ...
    public interface IDialogueView
    {
        ...
        /// </summary>
        /// <param name="speakerID">話者のID。View側での名前の切り替えや立ち絵、吹き出し市の特定に使用</param>
        /// <param name="cleanText">演出タグが取り除かれた、純粋に画面へ表示するテキスト</param>
        /// <param name="onTyepingComplete">文字表示がすべて完了した瞬間に、View側からマネージャーへ完了を伝えるコールバック</param>
        void DisplaySentence(string speakerID, string cleanText, Action onTypeingComplete);

        /// <summary>現在行っているタイピング演出を強制ストップし、全文を一瞬で表示する</summary>
        void ForceCompleteTyping();

        /// <summary>
        /// 選択肢を画面に生成してプレイヤに提示する
        /// </summary>
        /// <param name="choices">選択肢データのリスト</param>
        /// <param name="onChoiceSelected">プレイヤがボタンを押したときに、その要素番号(index)をマネージャに返すコールバック</param>
